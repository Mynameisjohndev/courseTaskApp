...
const UserContextProvider = ({children}: IContext) => {
  ...
  const handleSaveUser = (name: string) =>{
    if(name){
      ...
      Alert.alert('Ops!', 'Você precisa informar o seu nome de usuário para entrar no app');
    }
  };
  ...
  return(
    <UserContext.Provider value={{loading, user, initialRouteName, loginButtonRef, loadingSaveUser, handleSaveUser}}>
      {children}
